...
def _gen_sql(table_name, mappings):
    ...
    for f in sorted(mappings.values(), lambda x, y: cmp(x._order, y._order)):
        ...
        if f.foreign_key:
            ref = f.reference
            sql.append('foreign key(%s) references %s(%s),' % (ref[0], ref[1]))
        #sql.append(nullable and '  `%s` %s,' % (f.name, ddl) or '  `%s` %s not null,' % (f.name, ddl))
        sql.append('  `%s` %s %s,' % (f.name, ddl, auto_incre) if nullable else '  `%s` %s not null %s,' % (f.name, ddl, auto_incre))
    if pk1:
        sql.append('primary key(%s,%s),' % (p1[0], pk[1]))
...
class Field(object):
    ...
    def __init__(self, **kw):
        ...
        self.auto_incre = kw.get('auto_incre', False)
        ...
        if self.foreign_key:
            ...
            if not self.reference:
                raise exceptions.ValueError("None reference")
            elif type(self.reference) != list \
                and type(self.reference) != tuple:
                raise exceptions.ValueError(
                    "reference need list or tuple (table, column)")
            elif not self.reference[0] or not self.reference[1]:
                raise exceptions.ValueError("reference can't be empty")
